"""Side-effect-free readiness checks for reviewed OpenCLI executors."""
...
class OpenCLIRuntimeProbe:
    """Read-only daemon state used before a browser-backed adapter starts."""
    ...
    def public_result(self) -> dict[str, Any]:
        """Return non-sensitive state suitable for a JSON envelope."""
...
def select_opencli_profile(
    ...
    *,
    ...
    """Resolve the profile selected by reviewed arguments or process config."""
    ...
    *,
    ...
    try:
        ...
        return OpenCLIRuntimeProbe(
            ...
            detail=(
                f"daemon /status returned HTTP {exc.code}; "
                "OpenCLI will decide readiness"
            ),
        )
    ...
    return OpenCLIRuntimeProbe(
        ...
        extension_connected=None,
        detail=(
            "daemon /status omitted extensionConnected; OpenCLI will decide readiness"
        ),
    )


def opencli_error_code(
    stdout: str | None,
    stderr: str | None,
) -> str | None:
    """Extract a typed OpenCLI error code from JSON or YAML output."""
...
def is_browser_connect_failure(
    ...
    """Return whether OpenCLI reported its typed BROWSER_CONNECT failure."""
