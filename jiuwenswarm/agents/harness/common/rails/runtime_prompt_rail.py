...
class RuntimePromptRail(DeepAgentRail):
    ...
    async def before_model_call(self, ctx: AgentCallbackContext) -> None:
        ...
        if self._channel == "web":
            browser_tool_policy = (
                "# Browser Tool Policy\n\n"
                "- Automatic OpenCLI-first routing: `opencli-web` is preinstalled by default. For every task "
                "involving a live website, invoke it with the Skill tool before browser automation; do not "
                "search for, install, or ask the user to select it. If it lists an exact match for both the "
                "website and requested operation, read the bundled site module with `skill_name` set to "
                "`opencli-web` and `relative_file_path` set to the path listed by the root router. The "
                "site module is a router, not an execution contract. In the main agent, read exactly one "
                "listed operation module with the Skill tool, again using `skill_name=opencli-web` and its "
                "full `relative_file_path`, before invoking any adapter command. Follow only the loaded "
                "operation module's documented workflow. When it declares an `opencli_contract`, call "
                "`opencli_execute` immediately from this main Agent; the tool requires the exact operation "
                "read's short-lived, one-use, session/Agent-bound receipt and current file hash.\n"
                ...
                "or its matching site/operation module is unavailable or disabled, no exact operation "
                "contract is listed, or OpenCLI reports a provable pre-execution infrastructure failure. "
                "Never run OpenCLI and `browser_agent` concurrently for the same operation. Once an OpenCLI "
                "write process starts, a failure, timeout, or ambiguous result must not retry with "
                "`browser_agent`; use a supported read-only verification or stop and report uncertainty.\n"
                ...
                "for browser automation. For an operation declaring `opencli_contract`, `opencli_execute` "
                "is the only authorized execution entry; never invoke its Python wrapper or OpenCLI command "
                "directly. A not-yet-migrated operation may use only the exact legacy command documented by "
                "its loaded module. This does not allow direct browser launches or ad-hoc browser scripts.\n"
