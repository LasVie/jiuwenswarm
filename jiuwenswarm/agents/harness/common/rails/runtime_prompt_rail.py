...
class RuntimePromptRail(DeepAgentRail):
    ...
    async def before_model_call(self, ctx: AgentCallbackContext) -> None:
        ...
        if self._channel == "web":
            browser_tool_policy = (
                "# Browser Tool Policy\n\n"
                "- OpenCLI-first routing: for every task involving a live website, read `opencli-web` first "
                "with the Skill tool when that skill is available. If it lists an exact match for both the "
                "website and requested operation, read the matching OpenCLI site skill and follow only its "
                "documented adapter workflow before considering browser automation.\n"
                "- Use `task_tool` with `subagent_type` set to `\"browser_agent\"` only when `opencli-web` "
                "or its matching site skill is unavailable/disabled, no exact adapter operation is listed, "
                ...
                "`social_post_draft_select`. After draft selection, use the matching OpenCLI site skill when "
                ...
                "for browser automation. The only command-line exception is invoking an installed OpenCLI "
                "site skill's documented wrapper; this exception does not allow direct browser launches or "
                "ad-hoc browser scripts.\n"
