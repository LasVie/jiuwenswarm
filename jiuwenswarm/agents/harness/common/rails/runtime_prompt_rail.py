    ...
    async def before_model_call(self, ctx: AgentCallbackContext) -> None:
        ...
        if self._channel == "web":
            browser_tool_policy = (
                ...
                "`opencli-web` and `relative_file_path` set to the path listed by the router, then follow "
                "only that module's documented adapter workflow.\n"
                ...
                "loaded site module is self-contained and must not require another file-reading agent.\n"
                "- Use `task_tool` with `subagent_type` set to `\"browser_agent\"` only when `opencli-web` "
                "or its matching site module is unavailable/disabled, no exact adapter operation is listed, "
                "or OpenCLI reports a provable pre-execution infrastructure failure. Never run OpenCLI and "
                "`browser_agent` concurrently for the same operation. Once an OpenCLI write process starts, "
                "a failure, timeout, or ambiguous result must not retry with `browser_agent`; use a supported "
                "read-only verification or stop and report uncertainty.\n"
                ...
                "`social_post_draft_select`. After draft selection, use the matching OpenCLI site module when "
                "supported; otherwise use `browser_agent` to fill the current platform compose UI. In either "
                "path, stop before any externally visible publish action. Final publishing requires "
                "`social_post_confirm`; after confirmation, publish exactly once and only if the prepared "
                "state matches the confirmed context. If an OpenCLI publish attempt starts, it must not retry "
                "with `browser_agent`. `social_post_cancel` stops without publishing.\n"
                ...
                "adapter command or guarded wrapper documented by the loaded `opencli-web` site module; "
                "this exception does not allow direct browser launches or ad-hoc browser scripts.\n"
