...
| `usage` | `private_content_read` / `medium` | `opencli deepseek usage -f json`<br>Read DeepSeek platform usage: balance, cumulative spending, time-dimension spending, API requests, and Tokens. | none | auth=required; transport=browser_cookie; fallback_before=browser_agent; fallback_after=none |
...
- `usage`: sensitive output: private content, account identifiers
