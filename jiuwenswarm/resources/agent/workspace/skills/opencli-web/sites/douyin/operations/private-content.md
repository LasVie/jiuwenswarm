...
| `videos` | `private_content_read` / `medium` | `opencli douyin videos [--limit <limit>] [--page <page>] [--status "<all\|published\|reviewing\|scheduled>"] -f json`<br>获取作品列表 | `limit` (int, optional, default=20); `page` (int, optional, default=1); `status` (str, optional, default='all', choices=all,published,reviewing,scheduled) | auth=required; transport=browser_cookie; fallback_before=browser_agent; fallback_after=none |
