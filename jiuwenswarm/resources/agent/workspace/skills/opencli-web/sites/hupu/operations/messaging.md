...
|---|---|---|---|---|
| `reply` | `disabled` | `message_send` / `high` | Not executable; use the declared fallback if permitted<br>回复虎扑帖子 (需要登录) | `tid` (str, required, positional); `topic_id` (str, required); `text` (str, required, positional); `quote_id` (str, optional) |
