...
def build_a2ui_autonomy_instruction(language: str = "en") -> str:
    ...
    social_post_flow_rule_en = (
        ...
        "target audience. After selection, resume the normal website execution "
        "policy; A2UI collects and confirms data but does not choose between "
        "OpenCLI and browser_agent. Prepare the draft without publishing. The "
        "final confirmation action.context MUST preserve the confirmed fields "
        "and include execution_route='opencli' or 'browser_agent' for the route "
        "that actually prepared the draft, plus operation_contract when that route "
        "is OpenCLI. The final publish confirmation MUST "
        "use confirm action 'social_post_confirm' and cancel action "
        "'social_post_cancel'. After confirmation, use only that recorded route "
        "and publish once if the prepared state matches the context. Never publish, "
        "post, comment, like, follow, delete, or perform externally visible social "
